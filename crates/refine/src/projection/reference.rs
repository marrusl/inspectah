...
use super::types::{
    ...
    RefQuadletItem, RefRunningContainerItem, RefServiceAdvisory, RefServiceItem, RefServiceWarning,
    RefServices, RefStaticRoute, RefStorage, RefSysctlOverride, RefVarDirectory,
    RefVersionChangeItem, RefVersionChanges, ReferenceProjection,
};
...
pub fn project_ref_services(snap: &InspectionSnapshot) -> RefServices {
    ...
    for unit_name in &svc.enabled_units {
        if !divergent_set.contains(unit_name.as_str()) && !matched_set.contains(unit_name.as_str())
        {
...
pub fn project_ref_kernel_boot(snap: &InspectionSnapshot) -> RefKernelBoot {
    ...
    let map_snippets =
        |snippets: &[inspectah_core::types::kernelboot::ConfigSnippet]| -> Vec<RefConfigSnippet> {
            snippets
                .iter()
...
mod tests {
    ...
    fn test_divergent_service_categorized() {
        ...
        assert_eq!(result.divergent[0].owning_package, Some("firewalld".into()));
    ...
    fn test_preset_matched_with_dropin_visible() {
        ...
        assert_eq!(
            result.preset_matched_with_dropins[0].dropin_contents.len(),
            1
        );
    ...
    fn test_unknown_enabled_and_disabled_categorized() {
        ...
        assert_eq!(
            result.preset_unknown_enabled[0].unit,
    ...
    fn test_cmdline_preserved_untruncated() {
        let long_cmdline = "BOOT_IMAGE=(hd0,msdos1)/vmlinuz root=/dev/mapper/rhel-root ro crashkernel=auto \
    ...
    fn test_config_snippets_extracted() {
        ...
        assert_eq!(result.dracut_conf[0].content, "add_drivers+=\" iscsi \"");
    ...
    fn test_credential_refs_extracted() {
        ...
        assert_eq!(
            result.credential_refs[0].credential_path,
            "/etc/fstab.d/creds"
        );
    ...
    fn test_at_jobs_mapped() {
        ...
        assert_eq!(
            result[0].summary,
            Some("root: /usr/local/bin/cleanup".into())
        );
