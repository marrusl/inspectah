...
mod tests {
    ...
    fn script_safe_escapes_greater_than() {
        let input = r#"{"v":"a>b"}"#;
        let output = script_safe_json(input);
        assert!(!output.contains('>'), "output must not contain literal >");
        assert!(
            output.contains("\\u003e"),
    ...
    fn script_safe_preserves_non_special_content() {
        ...
        assert_eq!(
            input, output,
            "non-special content must pass through unchanged"
        );
    }
    ...
    fn filter_data_excludes_secrets() {
        ...
        assert!(!json.contains("ssh_keys"), "DTO must not contain ssh_keys");
        assert!(
