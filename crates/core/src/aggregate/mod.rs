...
/// Classify an aggregate prevalence into a zone based on its distribution across hosts.
///
...
pub fn classify_zone(prevalence: &AggregatePrevalence) -> PrevalenceZone {
...
/// Merge multiple host snapshots into a single aggregate snapshot.
///
...
pub fn merge_snapshots(
    ...
    merged.aggregate_meta = Some(aggregate_meta);
    merged.target_image = target_image;
    merged.baseline = baseline;
    merged.completeness = completeness;
    // Redaction state is per-host; aggregate merges don't carry it.
    ...
    // meta: union all host meta keys (first-writer-wins), then stamp aggregate provenance
...
mod tests {
    ...
    #[test]
    fn test_merge_stamps_aggregate_source_in_meta() {
    ...
    #[test]
    fn test_aggregate_merge_subscription_picks_latest_expiry() {
    ...
    #[test]
    fn test_aggregate_merge_subscription_hostname_tiebreak() {
        use crate::types::subscription::{SubscriptionFile, SubscriptionSection};
    ...
    #[test]
    fn test_aggregate_merge_subscription_mixed_presence() {
        use crate::types::subscription::{SubscriptionFile, SubscriptionSection};
