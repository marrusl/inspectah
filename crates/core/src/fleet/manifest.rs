...
impl FleetManifest {
    /// Parse a TOML string into a FleetManifest.
    pub fn parse(content: &str) -> Result<FleetManifest, toml::de::Error> {
        toml::from_str(content)
    }

    /// Load a manifest from a file, resolving source paths relative to the manifest's parent directory.
    pub fn load(path: &Path) -> Result<FleetManifest, Box<dyn std::error::Error>> {
...
"#;
    ...
    fn test_load_resolves_paths() {
        ...
        let m = FleetManifest::load(&manifest_path).unwrap();
