...
describe("DependencyModal", () => {
  ...
  it("calls onClose when close button clicked", async () => {
    const onClose = vi.fn();
    render(
      <DependencyModal packageId="httpd.x86_64" dependencies={deps} isOpen={true} onClose={onClose} />
    );
  ...
  it("renders nothing when not open", () => {
    const { container } = render(
      <DependencyModal packageId="httpd.x86_64" dependencies={deps} isOpen={false} onClose={vi.fn()} />
    );
  ...
  it("scrolls long dependency lists", () => {
    ...
    render(
      <DependencyModal packageId="httpd.x86_64" dependencies={longDeps} isOpen={true} onClose={vi.fn()} />
    );
