...
describe("AttentionSummary", () => {
  it("shows review count and repo count when needs_review > 0", () => {
    render(
      <AttentionSummary
        needsReviewCount={3}
        needsReviewRepoCount={2}
        infoCount={5}
        infoRepoCount={3}
      />,
    );
    expect(
      screen.getByText("3 packages need review across 2 repos"),
    ).toBeInTheDocument();
  });
  ...
  it("shows singular 'package' and 'repo' when counts are 1", () => {
    render(
      <AttentionSummary
        needsReviewCount={1}
        needsReviewRepoCount={1}
        infoCount={0}
        infoRepoCount={0}
      />,
    );
    expect(
      screen.getByText("1 package needs review across 1 repo"),
    ).toBeInTheDocument();
  });
  ...
  it("shows all-clear when both are 0", () => {
    ...
    expect(
      screen.getByText("All actionable items reviewed"),
    ).toBeInTheDocument();
  });
