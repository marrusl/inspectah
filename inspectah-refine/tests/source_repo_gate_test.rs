...
fn test_source_repo_populated_in_golden_data() {
    ...
    let content = fs::read_to_string(&path).expect("golden test data must exist");
    let rpm: RpmSection = serde_json::from_str(&content).expect("golden data must deserialize");
    let packages_with_repo: Vec<_> = rpm
        .packages_added
        .iter()
        .filter(|p| !p.source_repo.is_empty())
        .collect();
    assert!(
        !packages_with_repo.is_empty(),
        "golden data must have packages with non-empty source_repo"
    );
    let known_repos: Vec<_> = packages_with_repo
        .iter()
    ...
    assert!(
        known_repos.len() >= 2,
        "golden data should have multiple distinct repos, got: {:?}",
        known_repos
    );
}
