...
- [x] **`strip_prefix` vs `trim_start_matches` inconsistency:** npm global and system gem paths were stored with `strip_prefix('/')` in the collector while the other eight `nonrpm.rs` sites and all six renderer sites used `trim_start_matches('/')`. The original entry named the wrong direction: `trim_start_matches` is the convention, and `strip_prefix` was the outlier. Both collector sites now trim, and the renderer's six sites are consolidated into `absolute_path()` in `language_packages.rs`, whose doc comment states the convention. *(DONE — 2026-08-15)*
...
- [x] **Broader shell metachar rejection in renderer:** `language_packages.rs` rejected only `'` and `\n` in paths/packages. The guard now rejects `;`, `$`, backtick, `|`, and `&` as well, named in a `UNSAFE_SHELL_CHARS` constant the test table iterates so the two cannot drift apart. Matters most for npm global package names, which interpolate into `RUN npm install -g` unquoted. *(DONE — 2026-08-15)*
